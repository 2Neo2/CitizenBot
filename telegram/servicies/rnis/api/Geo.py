...
class Contract:
    ...
    @Request.All_pages
    @Request.Retry
    async def table(self, **d):
        '''update | Geo.Contract.update'''
        url  = self.URL.get(self.URL.Geo.Contract.table)
        json = Filters.Geo.Contract.table(**dict(d, token=Request.token(self.__session)))
        return await self.__session.post(url=url, json=json)
    ...
    async def export(self, **d):
        '''export | Geo.Contract.export'''
        url  = self.URL.get(self.URL.Geo.Contract.export)
        json = Filters.Geo.Contract.export(**dict(d, token=Request.token(self.__session)))
        return await self.__session.post(url=url, json=json)
