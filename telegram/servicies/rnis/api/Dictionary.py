...
class Dictionary:
    ...
    @Request.All_pages
    @Request.Retry
    async def carrier(self, **d):
        '''read | Dictionary.carrier'''
        url = self.URL.get(self.URL.Dictionary.carrier)
        json = Filters.Dictionary.carrier(**dict(d, token=Request.token(self.__session)))
        return await self.__session.post(url=url, json=json)