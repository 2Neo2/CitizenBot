    ...
    async def request(*args, **kwargs):
        ...
        if isinstance(response, dict):
            ...
            if total_pages > 1 and all_pages is True:
                ...
                items = isinstance(payload.get('items'), list)
                response.pop('payload')
                if items:
                    response['payload'] = payload
                if not(items):
                    response.setdefault('payload', {})['items'] = [payload]
                        ...
                        for r in await asyncio.gather(*tasks):
                            ...
                            if payload:
                                if not(payload.get('error', False)):
                                    if items:
                                        response['payload']['items'] += payload.get('items', [])
                                    else:
                                        response['payload']['items'].append(payload)
